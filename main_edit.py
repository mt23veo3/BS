# -*- coding: utf-8 -*-
"""
Main loop PRE -> PROBE -> FULL (OPTIMIZED VERSION)
Chống spam cảnh báo đóng lệnh: mỗi lệnh chỉ gửi đề xuất đóng lệnh khi lý do thay đổi hoặc lệnh mới.

TÍCH HỢP CÁC TỐI ƯU CHUYÊN GIA:
1. Linh hoạt xác nhận đa khung: M5 chỉ cấm khi ngược hướng, cho phép NEUTRAL
2. Tăng threshold score: M15=15, H1=8, adx_h1_threshold=28, heavy_required=3
3. Bộ lọc biên độ đảo chiều theo ATR: Chỉ vào lệnh nếu giá lệch với lệnh trước >= 0.7 * ATR
4. snapshot_m5_confirmed cần 3 nến đồng pha, chỉ cấm khi M5 ngược rõ
5. should_send_new_entry nâng lên 0.5%
6. Logging các trường hợp đảo chiều liên tiếp, bỏ qua tín hiệu nếu quá nhiều
"""
...
REVERSAL_TRACKER: Dict[Tuple[str, str], Dict] = {}  # Track consecutive reversals
...
def should_send_new_entry(symbol, timeframe, direction, entry, sl, tp, min_interval_min=15, min_entry_diff_pct=0.5):
    ...
    # Track reversals
    reversal_state = REVERSAL_TRACKER.get(key, {"count": 0, "last_reversal_ts": 0})
    
    if last:
        last_side = last.get("side")
        if last_side != direction:
            # Log consecutive reversal
            reversal_state["count"] = reversal_state.get("count", 0) + 1
            reversal_state["last_reversal_ts"] = now
            REVERSAL_TRACKER[key] = reversal_state
            
            warning_text = f"🚨 **CẢNH BÁO: ĐẢO CHIỀU #{reversal_state['count']}** {symbol} {timeframe} từ {last_side} sang {direction}"
            print(f"[REVERSAL] Đảo chiều lần thứ {reversal_state['count']} trong thời gian gần đây: {symbol} {timeframe}")
            
            # Skip signal if too many consecutive reversals (> 2 within 1 hour)
            if reversal_state["count"] >= 3 and (now - reversal_state.get("first_reversal_ts", now)) < 3600:
                print(f"[REVERSAL] BỎ QUA tín hiệu do quá nhiều đảo chiều liên tiếp ({reversal_state['count']} lần trong 1h)")
                return False, None
            
            if reversal_state["count"] == 1:
                reversal_state["first_reversal_ts"] = now
                REVERSAL_TRACKER[key] = reversal_state
        elif last_side == direction:
            # Same direction - reset reversal counter
            if reversal_state.get("count", 0) > 0:
                print(f"[REVERSAL] Reset bộ đếm đảo chiều cho {symbol} {timeframe}")
                REVERSAL_TRACKER[key] = {"count": 0, "last_reversal_ts": 0}
...
def check_atr_reversal_filter(symbol, timeframe, price_now, atr_val, min_atr_mult=0.7):
    """
    Bộ lọc biên độ đảo chiều theo ATR:
    Chỉ vào lệnh nếu giá lệch với lệnh trước >= 0.7 * ATR
    """
    key = (normalize_symbol(symbol), timeframe)
    last = SENT_SIDE.get(key)
    
    if last is None:
        # Không có lệnh trước, cho phép
        return True, 0
    
    last_entry = last.get("entry", 0)
    if last_entry == 0 or atr_val == 0:
        return True, 0
    
    price_diff = abs(price_now - last_entry)
    atr_threshold = min_atr_mult * atr_val
    
    if price_diff >= atr_threshold:
        print(f"[ATR_FILTER] PASS: Giá lệch {price_diff:.4f} >= {atr_threshold:.4f} ({min_atr_mult}*ATR)")
        return True, price_diff
    else:
        print(f"[ATR_FILTER] BLOCK: Giá lệch {price_diff:.4f} < {atr_threshold:.4f} ({min_atr_mult}*ATR), BỎ QUA tín hiệu")
...
def snapshot_m5_confirmed(side_m15, m5, ind_m5, count=PROBE_M5_COUNT):
    """
    Linh hoạt xác nhận M5:
    - M5 chỉ cấm khi ngược hướng rõ ràng
    - Cho phép NEUTRAL
    - Cần 3 nến đồng pha để xác nhận
    """
    ...
    # Chỉ cấm khi M5 ngược rõ (tất cả 3 nến đều ngược)
    if opposite_direction_count >= count:
        print(f"[M5_CONFIRM] M5 ngược hướng rõ ràng ({opposite_direction_count}/{count} nến), BỎ QUA tín hiệu M15")
        return False
    
    # Cho phép nếu có ít nhất 3 nến đồng pha hoặc NEUTRAL không cấm
    if same_direction_count >= count:
        print(f"[M5_CONFIRM] M5 xác nhận đồng pha ({same_direction_count}/{count} nến)")
        return True
    
    # Nếu có NEUTRAL hoặc hỗn hợp không rõ ràng, vẫn cho phép
    print(f"[M5_CONFIRM] M5 không rõ ràng, CHO PHÉP (đồng pha: {same_direction_count}, ngược: {opposite_direction_count})")
    return True
...
async def run_once(cfg: Dict[str, Any], notifier: Notifier):
    now_epoch = time.time()
    # Updated thresholds: M15=15, H1=8, adx_h1=28, heavy_required=3
    ...
    adx_h1_threshold = int(cfg.get("adx_h1_threshold", 28))  # Updated to 28
    ...
    for symbol in symbols:
        ...
        # Flexible M5 confirmation: only block when M5 is clearly opposite
        m5_allows = (side_m5 == side_m15) or (side_m5 == "NEUTRAL")
        if not m5_allows:
            print(f"[M5_GATE] M5 BLOCKING: M5={side_m5}, M15={side_m15} (M5 ngược hướng)")
        elif side_m5 == "NEUTRAL":
            print(f"[M5_GATE] M5 NEUTRAL allowed: M5={side_m5}, M15={side_m15}")
        ...
        # Debug log for gates evaluation
        if symbol in ["BTC/USDT", "ETH/USDT"]:  # Only log for major pairs to reduce spam
            print(f"[GATES] {symbol}: M15_score={m15_score:.1f}>={th_m15}, H1_score={h1_score:.1f}>={th_h1}, " +
                  f"heavy={hhits}>={heavy_required}, ADX_H1={adx_h1:.1f}>={adx_h1_threshold}, gates_ok={gates_ok}")
        ...
        if is_breakout_candle(m15, ind_m15, direction=probe_direction):
            if not active_probe and not active_full:
                # Check ATR reversal filter
                atr_filter_ok, price_diff = check_atr_reversal_filter(symbol, "15m", price_now, atr_val, min_atr_mult=0.7)
                
                if not atr_filter_ok:
                    print(f"[ATR_FILTER] Bỏ qua breakout {symbol} do không đủ biên độ đảo chiều (diff={price_diff:.4f}, cần >= 0.7*ATR={0.7*atr_val:.4f})")
                elif atr_filter_ok:
